...
```{r fig3A.kcv, fig.cap="Fig3A"}
  ...
  load("RData/dl.kcv.result.core17.rev1.RData") # Nat Comms Rev1 2025-04-24 (NB, -LPOCV)
  #########################################################
  # Reulst of 11 ML methods based on 5-fold CV with 5 rep #
  #########################################################
  (dt.foo<-lapply(dl.kcv.result, function(DT) DT[method!="ENet1",.(.N, 
  ...
  (dt.bar<-
    dt.foo[,.(.N,N_test=sum(N),rankSum=sum(Rank),meanRank=sum(Rank)/5, #F2-F6
            meanAUC=round(mean(mean_AUC_test)/100,3),
            meanAUC_lo=round(mean(mean_AUC_test_lo)/100,3),
            meanAUC_hi=round(mean(mean_AUC_test_hi)/100,3))
    ,method][order(rankSum)][method=="ENet2",method:="ENet"]
  )
  ...
  column_spec(5, image = spec_pointrange(
    ...
    vline = .80,
    height=50)
  ) %>% save_kable("Figures/cfRNA.Fig3.KCV.table.pdf")

  ##
  ##
  (dt.baz<-dt.bar[,.(Method=method,`Rank sum`=rankSum, `Mean rank`=meanRank,`Mean AUC (95% CI)`=paste0(sprintf("%.3f",meanAUC)," (",paste0(sprintf("%.3f",meanAUC_lo))," - ",paste0(sprintf("%.3f",meanAUC_hi)),")"))])

  library(ggpmisc)

  (dt.foo<-lapply(names(dl.kcv.result), function(NF) 
      dl.kcv.result[[NF]][method!="ENet1",.(NF=substr(NF,2,3),.N, 
  ...
  dt.foo[method=="ENet2",method:="ENet"]
...
```{r fig3B.final.model}
 ...
 library(ggbreak) 
  ...
  p.enet.lasso.tr<-ggplot(dt.enet.lasso.tr[Method=="ENet"], aes(N,Score,group=Measure)) +
    ...
    labs(x="Number of predictor mRNAs in the training model",y="Score") +
...
```{r fig5.LEP.PAPPA2.CPM, fig.cap="Fig5"}
  ...
  #
  pdf(file="Figures/cfRNA.Fig5A.pdf", width=11, height=8.5, title="Fig5: Abundance of LEP and PAPPA2")
  p5.lep.pappa2.sig
  dev.off()
...
## Figure 5B. Longitudinal Analysis via Mixed Effect Model
```{r cfrna-lmer1}
  # see longitudinal_analysis.Rmd
  ...
  #my.title.y<-latex2exp::TeX(r"($log_2 CPM \textbf{bold text}$)")
  ...
  #cowplot::plot_grid(p5.lep.pappa2.sig, p5.lmer, labels="AUTO", ncol=1, label_size=27, align="v", axis="l")
  p5.lmer
  dev.off()
...
```{r suppl.t.s6}
  ...
  #load("RData/dl.kcv.result.core17.RData")
  load("RData/dl.kcv.result.core17.rev1.RData") # Nat Comms Rev1 2025-04-24
  lapply(dl.kcv.result, nrow)
  xtabs(~fold+method, dl.kcv.result[["F2"]])
  dl.kcv.result[["F2"]][method=="glParallel"][order(-AUC)]

  dt.kcv<-rbindlist(dl.kcv.result)
  dt.kcv[,c("Fold","Rep"):=tstrsplit(fold,"\\.")] %>% setcolorder(c("N","Fold","Rep"))
  fwrite(dt.kcv[,-"fold"], file="Tables/Suppl/SI.Table6.by.fold.csv")
  #
```
...
```{r suppl.t.s7}
  ...
  # Nat Comms Rev1 2025-04-25
  load("RData/dl.final.models.core17.rev1.RData")
  dl.final.models[["F2"]]
  dl.final.models[["F2"]][order(method,feature)][,.(predictor=paste(feature,collapse=",")),method][order(predictor)]

  dt.bar<- lapply(names(dl.final.models), function(NF){
    dl.final.models[[NF]][order(method,feature)][,.(NF=substr(NF,2,3),predictor=paste(feature,collapse=",")),method]
  })  %>% rbindlist

  fwrite(merge(dt.foo, dt.bar)[order(NF,Rank)], file="Tables/Suppl/SI.Table7.csv")

  #
load("RData/dl.final.result.core17.RData")
dl.final.result[["F2"]][,.(methods,fold,predictor,AUC_test,LPOCV_test)][order(-AUC_test)]
dl.final.result[["F2"]][predictor=="HBQ1,PAPPA2"][,.(methods,fold,predictor,AUC_test,AUC_test_lo,AUC_test_hi)][order(fold)]
dl.final.result[["F2"]][predictor %in% c("HBQ1,PAPPA2","LEP,PAPPA2")][,.(methods,fold,predictor,AUC_test,LPOCV_test)] %>% dcast.data.table(methods+predictor~fold,value.var="AUC_test")

(lapply(names(dl.final.result), function(FN)
  dl.final.result[[FN]][grepl("glParallel",methods) & fold=="28wk(preterm)",-c("fold")][,`:=`(Method="LASSO",`N`=substr(FN,2,nchar(FN)) %>% as.integer)]
) %>% rbindlist)[,-"methods"]
```
...
+ ENet result
```{r suppl.t.s8}
  # Supplementary Table 8 – the final training models from ENet and LASSO
  load("RData/dl.enet.result.core17.RData")

  dl.enet.result[["F2"]][fold=="28wk(preterm)"]
  dl.final.result[["F2"]][fold=="28wk(preterm)"]
...
## Suppl Table S9
+ Validated AUC from combined model (preterm+term) vs. preterm dataset only
+ Compare the combined models vs those from the discovery model
```{r``{r suppl.t.s9}}
  load("RData/dl.enet.models.core17.RData") # ENet models from 28wk, discovery dataset (preterm)
  ...
  dl.enet2.models<-lapply(dl.enet.models, function(DT) DT[method=="ENet2"])

  lapply(names(li.num)[1:7], function(my.num.F) cbind(dl.combined.models[[my.num.F]][order(rank)], 
                                                 dl.enet2.models[[my.num.F]][order(rank)])
  )

  lapply(names(li.num), function(my.num.F) {dl.combined.models[[my.num.F]][order(feature)][,paste(feature,collapse=",")]})
  lapply(names(li.num), function(my.num.F) {dl.enet2.models[[my.num.F]][order(feature)][,paste(feature,collapse=",")]})

  ##
  ## AUC in training  (in-sample)
  ##
  cbind(
  (lapply(names(li.num), function(my.num.F) dl.combined.result[[my.num.F]][fold=="28wk(preterm+term)",.(Num=my.num.F,predictor,AUC_test,AUC_test_lo,AUC_test_hi)]) %>% rbindlist),
  (lapply(names(li.num), function(my.num.F) dl.enet.result[[my.num.F]][fold=="28wk(preterm)" & grepl("ENet",methods),.(Num=my.num.F,predictor,AUC_test,AUC_test_lo,AUC_test_hi)]) %>% rbindlist)
  )
  ...
  lapply(my.dataset, function(this.dataset){
    merge(
          (lapply(2:10, function(my.num) dl.combined.result[[my.num-1]][fold==this.dataset,.(Num=my.num,Predictor=predictor,AUC_test=AUC_test/100,AUC_test_lo=AUC_test_lo/100,AUC_test_hi=AUC_test_hi/100)]) %>% rbindlist),
          (lapply(2:10, function(my.num) dl.enet.result[[my.num-1]][fold==this.dataset & grepl("ENet",methods),.(Num=my.num,Predictor=predictor,AUC_test=AUC_test/100,AUC_test_lo=AUC_test_lo/100,AUC_test_hi=AUC_test_hi/100)]) %>% rbindlist),
    by="Num"
    )[,dataset:=this.dataset] %>% setcolorder("dataset")
  }) %>% rbindlist

  dt.compare.auc[dataset=="Munchel"]
  dt.compare.auc[dataset=="28wk(term)"]

  # Best10
  dt.compare.auc[order(-AUC_test.x)][1:10]
  dt.compare.auc[order(-AUC_test.y)][1:10]

  # Bottom10 
  dt.compare.auc[order(AUC_test.x)][1:10]
  dt.compare.auc[order(AUC_test.y)][1:10]

  fwrite(dt.compare.auc, file="Tables/Suppl/SI.Table.9.AUC.comparison.combined.vs.preterm.csv")
```
...
+ cfRNA changed by GA
```{r suppl.t.s10}}
  myRData="RData/dl.DEG.GA.control.term.POPS-2022.GRCh38.88.RData" # child-rmd/cfRNA.term.PE.DEG.GA.POPS-2022.GRCh38.88.Rmd
  load(myRData)

  dl.DEG.GA[["20wk-12wk"]][BH<0.05] # n=76
  dl.DEG.GA[["28wk-20wk"]][BH<0.05] # n=64
  dl.DEG.GA[["36wk-28wk"]][BH<0.05] # n=55

  dt.foo<-
  merge(
    dl.DEG.GA[["20wk-12wk"]][BH<0.05][,.(gene_id,log2FoldChange,pvalue)], # n=76
    dl.DEG.GA[["28wk-20wk"]][BH<0.05][,.(gene_id,log2FoldChange,pvalue)], # n=64
    by="gene_id",all=T
    ) %>% setnames(c("gene_id","log2FC(20wk/12wk)","BH(12wk_20wk)","log2FC(28wk/20wk)","BH(20wk_28wk)"))

  fwrite(
  merge(dt.foo,dl.DEG.GA[["36wk-28wk"]][BH<0.05][,.(gene_id,`log2FC(36wk/28wk)`=log2FoldChange,`BH(28wk_36wk)`=pvalue)],all=T), 
  file="Tables/Suppl/SI.Table.10.DESeq2.by.GA.csv")
```

## Suppl Table S11-S12
+ Mixed effect models
```{r suppl.t.s11}}
  # Supplementary Table 7 – r2 marginal and conditional for mixed effect models
  if(F){
  load("RData/li.models.lmer.RData") # see longitudinal_analysis.Rmd
  dt.R2<-(lapply(li.models, function(li.model) li.model[["stat"]]) %>% rbindlist)[order(-r2m)]
  dt.R2[,combined.p:=pchisq(-2 * (log(p.GA) + log(p.GA.sq)), 2 * 2, lower.tail = FALSE)]
  dt.R2[,BH:=p.adjust(combined.p,"BH")]
  fwrite(dt.R2[,-"p.intercept"], file="Tables/Suppl/SI.Table.8.lmer.csv")
  }

  load("RData/li.lmer.RData") # see longitudinal_analysis.Rmd
  ...
  fwrite(dt.R2.GA2, file="Tables/Suppl/SI.Table.11.lmer.GA2.csv")

  # GA
  dt.R2.GA<-dt.R2[Model=="GA"]
  dt.R2.GA[,BH.GA:=p.adjust(p.GA,"BH")]
  dt.R2.GA[,BH.anova:=p.adjust(p.anova,"BH")]
  fwrite(dt.R2.GA, file="Tables/Suppl/SI.Table.12.lmer.GA.csv")
```
...
## Suppl Data Figure 2 - combined model 
+ Validated AUC from combined model (preterm+term) vs. preterm dataset only
+ Compare the combined models vs those from the discovery model
```{r``{r suppl.fig.xx}}
  load("RData/dl.enet.result.core17.RData") # Validation of ENet models from 28wk, discovery dataset (preterm)
  ...
  xtabs(~Dataset+Model,dt.auc.long)
  dt.auc.long[Dataset=="Munchel"]
  dt.auc.long[Dataset=="28wk(preterm)"]
  dt.auc.long[Dataset=="28wk(term)"]
  dt.auc.long$Source
  dt.auc.long$Model

  p.auc.combined<-ggplot(dt.auc.long, aes(Num, AUC_test,group=Model)) + 
    ...
    ylab("AUC") + xlab("Number of predictor mRNA in the training models") +
    scale_color_manual(values=c("grey30",cbPalette2[2])) +
    facet_grid(Source~GA) + 
    theme_Publication() +
    theme(legend.position="top", panel.border = element_rect(colour = "black"))

  pdf(file="Figures/Suppl/cfRNA.Suppl.Data.Fig.combined.AUC.pdf", width=14, height=10, title="Suppl Fig: AUC from the combined model vs preterm model")
  p.auc.combined
  dev.off()
  ...
  # now add the title
  x.label<- cowplot::ggdraw() +
    cowplot::draw_label(
    "Number of predictor mRNA in the training models",
    fontface = 'bold',
    x = 0,
    hjust = 0
  ) +
...
## Suppl Data Figure - longitudinal analysis via `lme4`
+ See longitudinal_analysis.Rmd

```{r suppl.data.fig1, eval=F}
  library(grid)
  library(gridExtra)

  # Supplementary Data Figure - logCPM by GA via mixed effect model 
  load("RData/li.lmer.RData") # see longitudinal_analysis.Rmd
  load("RData/dt.cpmZ.term.POPS-2022.GRCh38.88.RData") # dt.cpmZ.term (term)
  ...
  dt.pops.sample<-merge(dt.pops.sample, dt.foo)
  dt.logCPM<-merge(dt.cpmZ.term[Condition=="Control"],dt.pops.sample[,.(SampleID,POPSID,GAwk)],by="SampleID")

  dt.logCPM[,rGAwk:=(GAwk-4)/8] # to make 12wk(1), 20wk(2), 28wk(3), and 36wk(4)
  dl.logCPM<-split(dt.logCPM, dt.logCPM$geneName)

  li.plots<-lapply(dl.logCPM[dt.R2[order(-r2m)][["gene"]]], function(dt.foo){
      my.gene<-unique(dt.foo[["geneName"]])
      message(my.gene)

      r2m<-li.models[[my.gene]][["stat"]][["r2m"]] %>% round(3)
      r2c<-li.models[[my.gene]][["stat"]][["r2c"]] %>% round(3)
      my.title=paste0(my.gene," (r2m:",r2m,", r2c:",r2c,")")
  
      ggplot(dt.foo, aes(x = GAwk, y = logCPM)) +
      geom_line(aes(group = POPSID), linewidth = .5, col="grey80",alpha=.7) +
      geom_point(aes(color = as.factor(GA)), size=2, alpha=.7) +
      labs(subtitle=my.title, x = "Gestational Age (weeks)", color = "GA (week)") +
      theme(legend.position = "top") +
      geom_ribbon(data=li.models[[my.gene]][["predict"]], aes(ymin=conf.low, ymax=conf.high), alpha=0.6, fill = "grey40") +
      geom_line(data=li.models[[my.gene]][["predict"]], linewidth=1.5, col="blue",alpha=0.8) + 
      scale_x_continuous(breaks=c(12,20,28,36)) +
      scale_color_manual(values=cbPalette2) +
      theme_Publication() +
      theme(legend.position="none")
  ...
  file.name1<-file.path('Figures/Suppl',paste0('cfRNA.Suppl.Data.Fig.logCPM_by_GA_lmer.top100.pdf'))
  pdf(file=file.name1, width=14, height=9,title="Longitudinal analysis via mixed effect model")
  grid.text(label=paste0("Longitudinal analysis of cfRNA abundance\nfrom maternal blood\n(Top 100 genes by R2)"),gp=gpar(fontsize=50,fontface="bold"),x=.5,y=.7,just="center")
  grid.text(label=paste("Sungsam Gong\n<ssg29 @ cam.ac.uk>",Sys.Date()),gp=gpar(fontsize=30,fontface="bold"),x=0.8,y=.2,just="right")
  gridExtra::marrangeGrob(li.plots[dt.R2[order(-r2m)][["gene"]][1:100]],nrow=2,ncol=4,top=NULL)
  dev.off()
```
...
## Correlation between LEP PAPPA2 
```{r}
  load("RData/dt.cpmZ.preterm.POPS-2022.GRCh38.88.RData") # dt.cpmZ (preterm)
  load("RData/dt.cpmZ.term.POPS-2022.GRCh38.88.RData") # dt.cpmZ.term (term)
  load("RData/dt.cpmZ.munchel.RData") # dt.cpmZ.munchel (Munchel)

  dt.foo<-dt.cpmZ[geneName %in% c("LEP","PAPPA2")] %>% dcast.data.table(SampleID+GA+Condition~geneName, value.var="logCPMZ")
  xtabs(~Condition+GA,dt.foo)
  split(dt.foo, dt.foo$GA)[1:3]
  lapply(split(dt.foo, dt.foo$GA)[1:3], function(DT) DT[,.(Cor=cor(LEP,PAPPA2),Pval=cor.test(LEP,PAPPA2)$p.value)])

  #  0.5226683 1.50778e-06
  #  0.5588601 2.281226e-07
  #  0.6285384 3.393297e-09

  dt.cpmZ

  p.corr<-
  ggplot(dt.foo, aes(LEP,PAPPA2)) +
    geom_point(size=2,shape=21,alpha=.8) +
    facet_wrap(~GA) +
    theme_Publication()

  cairo_pdf(file="Figures/Suppl/cfRNA.Suppl.LEP.PAPPA2.cor.pdf", width=10,height=4, onefile=T)
  p.corr
  dev.off()

  #
  li.mat<-lapply(split(dt.cpmZ, dt.cpmZ$GA), function(DT){
    DT %>% dcast.data.table(SampleID~geneName, value.var="logCPMZ") %>% as.matrix(rownames="SampleID")
  })
  li.mat[[1]][1:5,1:5]
  cor(li.mat[[1]])["LEP","PAPPA2"]

  ##
  ##
  ##
  #dt.cpmZ[geneName %in% c("LEP","PAPPA2")] %>% dcast.data.table(SampleID+GA+Condition~geneName, value.var="logCPMZ")
  dt.lep.pappa2<-rbind(
     ...
     ,fill=T)
  dt.lep.pappa2[,.N,.(Source,GA)]

  dt.lep.pappa2[,Dataset:=ifelse(is.na(GA),Source,paste0(Source,"(",GA,")"))]
  xtabs(~Dataset,dt.lep.pappa2)
  xtabs(~GA+Source,dt.lep.pappa2)
