...
```{r setup, include=FALSE, message=FALSE, warning=FALSE}
  ...
  li.methods<-list(
    `AdaBoost.M1`="AdaBoost.M1", #2025-04-23
    `svmLinear`="svmLinear",
    `svmRadial`="svmRadial",
    `nnet`="nnet",
    `pcaNNet`="pcaNNet",
...
get_cv_glm2<-function(x=li.mat[["test"]][["28wk"]],my.fold, my.model,my.feature){
    ...
    LPOCV.boot.test<-boot::boot(data=df.mat.test, statistic=get_LPOCV_boot,R=100,parallel="multicore",ncpus=10)
    # deal with failed boot result
    if(is.numeric(LPOCV.boot.test$t)){
      LPOCV.ci.test<-boot::boot.ci(LPOCV.boot.test,type="perc")
      LPOCV_test_lo=LPOCV.ci.test$percent[4]
    ...
    cbind(
      ...
      data.table(
                ...
                `AUC_test_lo`=my.roc.test$ci[1]*100, 
                `AUC_test_hi`=my.roc.test$ci[3]*100,
                `LPOCV_test`=NA, #LPOCV.boot.test$t0,
                `LPOCV_test_lo`=NA, #LPOCV_test_lo,
                `LPOCV_test_hi`=NA, #LPOCV_test_hi,
                 `AIC`=my.model$aic,
                 `BIC`=BIC(my.model)
                )
    )
...
#    `nbFuncs`=nbFuncs
  ...
  dl.kcv.rev1<-lapply(li.num, function(my.num){
  if(F){
    # 1. glParallel
    ...
    dt.cv.rfe # 2025-04-23 for Nat Comm rev1
    #rbind(dt.cv.glp, dt.cv.lasso[method=="LASSO"], dt.cv.enet, dt.cv.svm, dt.cv.rfe)
  }) # end of dl.kcv.rev1

  #save(dl.kcv, file=my.RData) # 2025-04-23

  # 2025-04-23
  if(T){
  dl.kcv<-
  lapply(structure(names(dl.kcv), names=names(dl.kcv)), function(Fx){
    rbind(dl.kcv[[Fx]],dl.kcv.rev1[[Fx]])
  })
  my.RData<-file.path("RData/dl.kcv.core17.RData")
  save(dl.kcv, file=my.RData) # 2025-04-23
  }
...
if(file.exists(my.RData)){
  ...
  # 2025-04-23
  my.RData<-file.path("RData/dl.kcv.result.core17.rev1.RData")
...
#    `nbFuncs`=nbFuncs
  ...
  if(T){
  dl.final.models<-
  lapply(structure(names(dl.final.models), names=names(dl.final.models)), function(Fx){
    rbind(dl.final.models[[Fx]],dl.final.models.rev1[[Fx]])
  })
  my.RData<-file.path("RData/dl.final.models.core17.rev1.RData")
  save(dl.final.models, file=my.RData) # 2025-04-23
  }
} # end of if
...
## Combined models from Discovery+Validation dataset (i.e. all POPS)
+ 2025-05-08
+ To address reviewer1's comment: " The authors may want to consider re-analysing the data without splitting the data set and use the complete POP(28 weeks) sample set for model development as advised (7)"
```{r combined_model_all_pops1}
if(F){
  li.mat[["train"]][["28wk"]] %>% dim
  li.mat[["test"]][["28wk"]] %>% dim

  li.mat[["train"]][["28wk"]][1:2,] 
  li.mat[["test"]][["28wk"]][1:2,] 
}

  li.num<-2:10 %>% as.list; names(li.num)=paste0("F",2:10)

  # combine 28wk from discovery (preterm) and validation (term) dataset
  ...
  ##
  ## LEP, PAPPA2, and LEP+PAPPA2
  my.targets<-c("LEP","PAPPA2","LEP,PAPPA2")
  dt.combined.result.small<-lapply(my.targets, function(my.proteins){
                        ############################################
                        # fit the model using the training dataset #
                        ############################################
                        my.feature<-strsplit(my.proteins,",") %>% unlist
  ...
  dt.combined.result.small[fold=="Munchel"]
```
